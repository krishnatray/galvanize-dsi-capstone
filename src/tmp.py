...
def get_train_test(filepath):
    ...
    if len(sys.argv) > 1 and int(sys.argv[1]) > 0:
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            data.corpus[:sample_size], y, test_size=0.25, random_state=0
        )
    else:
        # split into train test
        print("Class counts:")
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            data.corpus, y, test_size=0.25, random_state=0
        )
...
def profit_curve_main(filepath, cost_benefit):
    ...
    for model in models:
        print(model)
        profits, thresholds = get_model_profits(
    ...
    print(
        reporting_string.format(
            max_model.__class__.__name__, max_thresh, max_profit, proportion_positives
        )
    )
    ...
    print(
        ...
        accuracy_score(y_train, model.predict(X_train), normalize=True),
    )
...
def undersample(X, y, tp):
    ...
    negative_keepers = np.random.choice(
        a=[False, True],
        size=neg_count,
        p=[1 - negative_sample_rate, negative_sample_rate],
    )
...
def oversample(X, y, tp):
    ...
    positive_idxs = np.random.choice(
        a=positive_range, size=int(positive_size), replace=True
    )
...
def sampling_main(model, filepath, cost_benefit, range_params=(0.35, 0.65, 0.05)):
    ...
    for sampling_techinque, name in zip(
        [undersample, oversample, smote], ["undersampling", "oversampling", "smoting"]
    ):
